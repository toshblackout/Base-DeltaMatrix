...
junim: async (jid, content, options = {}) => {
try {
const seloMeta = {key: { fromMe: false,participant: '13135550002@s.whatsapp.net', id: "META-" + Date.now() }, message: { contactMessage: { displayName: 'Meta IA', vcard: 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + `FN:Meta IA\n` + `ORG:Meta IA;\n` + `TEL;type=MSG;type=CELL;type=VOICE;waid=13135550002:+13135550002\n` + 'END:VCARD' } } };

let messagePayload = {};

if (content.text) { 
messagePayload.extendedTextMessage = { text: content.text };
} else if (content.image) { 
messagePayload.imageMessage = { url: content.image, caption: content.caption || "" };
} else {
...
if (options.ai) {
const ctx = {
quotedMessage: seloMeta.message,
participant: seloMeta.key.participant,
stanzaId: seloMeta.key.id
};
const typeKey = Object.keys(messagePayload)[0];
messagePayload[typeKey].contextInfo = ctx;
}

const msg = { key: { remoteJid: jid, fromMe: true, id: "Pedrozz-" + Date.now() }, message: messagePayload };

await relayMessage(jid, msg.message, { messageId: msg.key.id });

return msg;
} catch (e) {
console.error("Erro no junim:", e);
throw e;
}
},
